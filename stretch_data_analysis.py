...
def p50_current_cutoff_plot(df, cutoff_list):
    for i in range(len(cutoff_list)):
        ...
        for name, group in grouped:
            ...
            # min_current = group['current']
            # print(str(group['current']).min())
            if group['current'].max() > cutoff:
                df_cut = pd.concat([df_cut, group], ignore_index=True)
        ...
        sns.pointplot(data = df_cut, x = 'mmHg', y = 'norm_i', hue = 'condition', ci = 'sd', estimator=np.mean,
        scale = 0.7, errwidth = 1, capsize = 0.5, legend = False, palette=pal_dict)
        plt.show()
...
cutoff_plots = p50_current_cutoff_plot(df, (20, 30, 40)) #cleaned
resistances = pip_resistance(df) #cleaned
#############################################################################################################
print(df)

sns.pointplot(x = 'mmHg',  y = 'norm_i', data = df, hue = 'condition')
plt.show()

cutoff_list = [20, 30, 40, 50, 60]
